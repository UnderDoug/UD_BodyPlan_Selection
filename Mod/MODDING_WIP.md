...
The first most straight forward method of customizing how an anatomy apears in the Body Plan Module and the Summary Block is by adding some display text.

Symbols can be added to the anatomy name (`Description`) and text can be added to the information readout (`LongDesctription`), with different versions between the Body Plan Module and the Summary Block that appears in the Summary Module.
...
Adding the `DescriptionAddition` tag will display a message towards the top of the info panel of the Body Plan Module, after a couple of native messages, but before "Includes the following body part slots:".

    <tag Name="DescriptionAddition" Value="This body plan is {{B|wonky}}, and you'll fall over, a lot!" />

Adding the `SummaryAddition` tag will display a message at the bottom of the Summary Block of the Summary Module. There's a very limited amount of text that can go here, and most of the messages will be truncated in favour of displaying limb type counts, so important or integral information shouldn't be put here.

    <tag Name="SummaryAddition" Value="{{B|Wonky, lots of falling!}}" />

Adding the `Symbols` tag will display a symbol after the name of the given anatomy and can be used as a short-hand representation of the other two tags. The `Symbols` tag takes Caves of Qud's usual string dictionary syntax
